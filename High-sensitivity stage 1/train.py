...
parser.add_argument("--alpha", type=float, default=1, help="The opacity of the watermark")
...
def main():
    ...
    event_dir = os.path.join("runs",tensorboard_name)
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            if step % 10 == 0:
                ...
                writer.add_image('gt', imgn_train, step)
        ...
        with torch.no_grad():
            for k in range(len(dataset_val)):
                ...
                random_img = random.randint(1, 2)
                imgn_train = add_watermark_noise(img_train, 0, True, random_img, alpha=opt.alpha)
